...
var container = d3.select('#chart-02'); // Create our scales

var xPositionScale = d3.scaleLinear().domain([2000, 2019]).range([0, width]);
var yPositionScale = d3.scaleLinear().domain([0.0, 1.0]).range([height, 0]); // Create a line generator
...
d3.csv(require('../data/internet_users.csv')).then(ready).catch(function (err) {
...
function ready(datapoints) {
  ...
  container.selectAll('svg').data(nested).enter().append('svg').attr('class', function (d) {
    return d.key.toLowerCase().replace(/[^a-z]*/g, '');
  }).attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').each(function (d) {
    ...
    svg.append('path').datum(datapoints).attr('d', line2).attr('stroke', 'red').attr('stroke', function (d) {
      if (name === 'Iran') {
        return 'red';
      } else {
        return '#4C8DD6';
    ...
    }).text(name).attr('font-weight', function (d) {
      if (name === 'Iran') {
        return 700;
      } else {
        return 700;
    ...
    }).attr('font-size', 12).attr('fill', '#333333').attr('x', xPositionScale('2019')).attr('dx', 5).attr('y', function (d) {
    ...
    var xAxis = d3.axisBottom(xPositionScale).tickValues([2000, 2019]);
    xAxis.tickFormat(d3.format(''));
    svg.append('g').attr('class', 'axis x-axis').attr('transform', 'translate(0,' + height + ')').call(xAxis).lower();
    var yAxis = d3.axisLeft(yPositionScale).tickValues([0.0, 0.5, 1.0]);
    yAxis.tickFormat(d3.format('.0%'));
    svg.append('g').attr('class', 'axis y-axis').call(yAxis).lower();
  });
}
},{"d3":"../node_modules/d3/index.js","../data/internet_users.csv":"data/internet_users.csv"}]},{},["scripts/chart-03.js"], null)
